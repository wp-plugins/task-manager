...
<li class="point-<?php echo $point->comment_ID; ?> wpeo-task-point" data-id="<?php echo $point->comment_ID; ?>" >
	<ul>
		...
		<li class="wpeo-point-input">
			<textarea data-nonce="<?php echo wp_create_nonce( 'wp_nonce_update_point' ); ?>" data-id="<?php echo $point->comment_ID; ?>" name="points[<?php echo $point->comment_ID; ?>]" type="text" <?php echo (!$task->informations->user_can_write) ? 'disabled' : ''; ?> <?php if( !empty( $point->informations->done ) ) : ?>class="wp-input-disabled" disabled="disabled"<?php else: ?>class="wpeo-point-can-update"<?php endif; ?>><?php echo $point->comment_content; ?></textarea>
		</li>
		<li>
			<?php if( empty( $point->informations->done ) && $task->informations->user_can_write ) : ?>
			...
			<?php if( empty( $point->informations->done ) && $task->informations->user_can_write ) : ?>
					<a tabindex="-1" href="<?php echo wp_nonce_url( admin_url( 'admin-ajax.php' ) . '?inlineId=thickbox_id&action=view-user&type=point&task_id=' . $task->ID . '&point_id=' . $point->comment_ID, 'view_user' ); ?>" title="Task #<?php echo $task->ID; ?> -> Point #<?php echo $point->comment_ID . ' : ' . __( 'Users', 'wpeotasks-i18n' ); ?>" class="my-thickbox dashicons dashicons-admin-users"></a>
